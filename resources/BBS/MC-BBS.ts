...
namespace BBS{
    ...
    export async function Fetch_BBS_JSON_DATA(url:string){
    ...
    // 生成论坛状态(badgeStatus)
    function set_badge_status(data:BBS_TEXT){
        ...
        elem.getElementsByTagName('span')[1].appendChild(add_bbs_type_info(BBS_TYPE[data["bbs-type"]]['title'],BBS_TYPE[data['bbs-type']]['content']));
        return elem;
    }

    // 生成论坛信息(mainInfo)
    function set_main_info(data:BBS_TEXT){
    ...
    // 生成论坛链接(btn-group)
    function set_btn_group(data:BBS_TEXT){
        // 复制链接按钮
    ...
    export function create_bbs_container(data:BBS_TEXT){
        // HTML框架
        // 每个论坛的外围框架
